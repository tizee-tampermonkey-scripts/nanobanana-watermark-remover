...
(function() {
    ...
    const isValidGeminiImage = (img) => {
        const isValid = img.src && img.src.includes('googleusercontent.com') &&
            (img.closest('generated-image') || img.closest('.generated-image-container') || img.alt?.includes('Generated image'));
        console.debug(`[Gemini Downloader] isValidGeminiImage:`, {
            src: img.src?.substring(0, 100) + '...',
            hasGoogleusercontent: img.src?.includes('googleusercontent.com'),
            hasGeneratedImageParent: !!img.closest('generated-image'),
            hasGeneratedImageContainer: !!img.closest('.generated-image-container'),
            altIncludesGenerated: img.alt?.includes('Generated image'),
            isValid
        });
        return isValid;
    };
    ...
    const findGeminiImages = () => {
        ...
        console.debug(`[Gemini Downloader] findGeminiImages: Found ${allImages.length} total images`);
        const validImages = allImages.filter(isValidGeminiImage);
        console.debug(`[Gemini Downloader] findGeminiImages: Filtered to ${validImages.length} valid Gemini images`);
        return validImages;
    };
    ...
    // --- UI Logic ---
    function createUI() {
        const existingFab = document.querySelector('.gemini-downloader-fab');
        if (existingFab) {
            console.debug(`[Gemini Downloader] createUI: FAB already exists, returning`);
            return;
        }

        console.debug(`[Gemini Downloader] createUI: Creating new FAB UI`);
        ...
        console.debug(`[Gemini Downloader] createUI: FAB successfully added to DOM`);
    }
    ...
    const updateVisibility = () => {
        console.debug(`[Gemini Downloader] updateVisibility: Checking visibility`);
        const images = findGeminiImages();
        const fab = document.querySelector('.gemini-downloader-fab');
        console.debug(`[Gemini Downloader] updateVisibility:`, {
            hasFab: !!fab,
            imageCount: images.length,
            willShowFab: images.length > 0
        });
        if (fab) {
            fab.style.display = images.length > 0 ? 'flex' : 'none';
            console.debug(`[Gemini Downloader] updateVisibility: Fab display set to ${fab.style.display}`);
        } else {
            console.debug(`[Gemini Downloader] updateVisibility: FAB element not found in DOM`);
        }
    ...
    const init = async () => {
        console.debug(`[Gemini Downloader] init: Starting initialization`);
        injectStyles();
        console.debug(`[Gemini Downloader] init: Styles injected`);

        try {
            engine = await WatermarkEngine.create();
            console.debug(`[Gemini Downloader] init: Watermark engine created successfully`);
        } catch (e) {
            console.error(`[Gemini Downloader] init: Failed to create watermark engine`, e);
        }

        createUI();
        console.debug(`[Gemini Downloader] init: UI creation triggered`);

        updateVisibility();
        console.debug(`[Gemini Downloader] init: Visibility updated`);

        const observer = new MutationObserver(debounce(updateVisibility, 300));
        observer.observe(document.body, { childList: true, subtree: true });
        console.debug(`[Gemini Downloader] init: Mutation observer started`);
        console.debug(`[Gemini Downloader] init: Initialization complete`);
    };
